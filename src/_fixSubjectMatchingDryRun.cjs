...
for (const subject of missingSubjects) {
  ...
  const assigned = career ? career.title : 'NO CAREER'
  console.log(subject.id, subject.label, subject.category, '=>', assigned)
  if (career && !career.matchedSubjects.includes(subject.id)) {
...
for (const career of careers) {
  if (Array.isArray(career.matchedSubjects)) {
    career.matchedSubjects.forEach((subj) => finalSubjectSet.add(subj))
  }
}
const stillMissing = subjects.filter((subject) => !finalSubjectSet.has(subject.id))
console.log('stillMissing count', stillMissing.length)
stillMissing.forEach((s) => console.log(s.id, s.label, s.category))
