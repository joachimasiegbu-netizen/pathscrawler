...
function chooseCareer(subject) {
  ...
  if (subject.category === 'Vocational' || subject.category === 'BTEC' || subject.category === 'T-Level' || subject.category === 'Refugee Vocational') {
    return careers.find((career) => career.title === 'Project Manager') || careers[0]
  }
  if (subject.category.startsWith('Refugee')) {
...
for (const subject of subjects) {
  ...
  console.log(subject.id, '=>', career ? career.title : 'NO CAREER', career ? career.category : '')
}
