...
function addGenericCareer(template) {
  ...
  const career = { ...template, id: Math.max(...careers.map((career) => career.id)) + 1, matchedSubjects: [...template.baseSubjects] }
  careers.push(career)
...
for (const career of careers) {
  if (Array.isArray(career.matchedSubjects)) {
    for (const subj of career.matchedSubjects) existing.set(subj, career)
  }
}

const missing = subjects.filter((subject) => !existing.has(subject.id))

const pairs = missing.map((subject) => {
  const career = chooseCareer(subject)
  return { subject, career, careerHasId: career && career.matchedSubjects && career.matchedSubjects.includes(subject.id) }
})

for (const pair of pairs) {
  if (!pair.career) {
    console.log('NO CAREER', pair.subject.id, pair.subject.label, pair.subject.category)
  } else if (!pair.careerHasId) {
    console.log('ASSIGN', pair.subject.id, pair.subject.label, pair.subject.category, '=>', pair.career.title, pair.career.category)
  }
}

const assignedCareerCounts = {}
for (const pair of pairs) {
  if (pair.career) {
    assignedCareerCounts[pair.career.title] = (assignedCareerCounts[pair.career.title] || 0) + 1
  }
}
console.log('pairs', pairs.length)
console.log('assigned career counts', assignedCareerCounts)
